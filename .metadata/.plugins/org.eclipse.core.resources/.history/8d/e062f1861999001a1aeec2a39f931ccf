...
//		asiakas.setAsiakas_id(jsonObj.getInt("asiakas_id"));
	...
	protected void doDelete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("polku: " + pathInfo);
		int asiakas_id = 0;
		String poistettavaAsiakas_id = String.valueOf(asiakas_id); 
		poistettavaAsiakas_id = pathInfo.replace("/", "");
		int poistettavaAsiakas_id_ = Integer.parseInt(poistettavaAsiakas_id);
		response.setContentType("application/json");
		PrintWriter out = response.getWriter();
		Dao dao = new Dao();			
		if(dao.poistaAsiakas(poistettavaAsiakas_id_)) {
			out.println("{\"response\":1}");
		} else {
			out.println("{\"response\":0}");
		}
